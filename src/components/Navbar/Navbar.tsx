                        ...
                        {[1, 2, 3].map((value) => (
                            <span
                                ...
                                className={`${styles["menu__burger-line"]} ${
                                    isMenuExpanded &&
                                    styles["menu__burger-line--active"]
                                }`}
                            ></span>
                        ))}
                    </div>
            ...
            <div className={`${styles.menu}`}>
                <ul
                    className={`${styles["menu__list"]} ${
                        !isMenuPath
                            ? styles["menu__list--normal"]
                            : styles["menu__list--active"]
                    } ${isMenuExpanded && styles["menu__list--active"]}`}
                >
                    {isMenuPath
                        ? menuPDF.map(({ id, title }, index) => (
                              <li
                                  key={id}
                                  className={`navbar-list-item ${
                                      ...
                                      hasScrollMoved &&
                                      styles["menu__item--sticky"]
                                  }`}
                                  onClick={() =>
                                      window.open(
                                          `/documents/${id}-2025.pdf`,
                                          "_blank"
                                      )
                                  }
                              >
